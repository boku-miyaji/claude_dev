...
| 情報収集 | **「情報収集」「最新動向」「ニュース」「速報」「動向」**（外部情報を網羅して監視） | 速報レポート作成（24h縛り） |
| リサーチ | **「分析」「比較」「競合」「市場調査」「どうすべきか」**（外部情報を使った特定の問いへの深掘り） | 調査レポート作成（仮説→検証→提言） |
| 調査部 | **「なぜ」「原因」「障害」「不具合」**（自社システムの内部問題） | 根本原因分析・再発防止策 |

**情報収集 vs リサーチ vs 調査部 の線引き（IMPORTANT）:**

- **情報収集部** = 外部・**監視**（monitoring）。「何が起きたか」を網羅的に毎日届ける
- **リサーチ部** = 外部・**深掘り**（investigation）。「どうすべきか」の問いに仮説→検証→提言で答える
- **調査部** = 内部・**根本原因分析**。「自社システムでなぜこれが起きたか」

混同例（避ける）:
- ❌ 情報収集部に「示唆を出して」「3段構成で書いて」「比較して」 → これはリサーチ部の仕事
- ❌ リサーチ部に「最新動向まとめて」 → これは情報収集部の仕事
- ❌ 情報収集部やリサーチ部に「自社の障害原因を調べて」 → これは調査部の仕事

**部署委譲時のプロンプト原則（IMPORTANT・事故防止）:**

部署 agent を Agent ツールで起動するとき、秘書は **prompt を最小化する**:

- ❌ **禁止**: 部署 CLAUDE.md にあるはずの規約（フォーマット・期間・カテゴリ・3段構成 等）を秘書側で再記述しない
- ❌ **禁止**: 別部署の規約を当該部署の prompt に混ぜない（例: リサーチ部の3段構成を情報収集部に書かない）
- ❌ **禁止**: 出力先パスを秘書が勝手に指定しない（部署 CLAUDE.md が正規パスを定義している）
- ✅ **推奨**: 社長の元発言 + 必要最小限のコンテキスト（PJ名・関連ファイル）のみ
- ✅ **推奨**: 「[部署名] の CLAUDE.md に従って実行してください」と明示

例（社長が「情報収集して」と言った場合）:

```
description: 情報収集（社長依頼）
subagent_type: 情報収集部
prompt: |
  社長の依頼: 「情報収集して」

  情報収集部の CLAUDE.md に従って実行してください。
  （24h 縛り、各社レポート・注目論文・focus-you/hd-ops 示唆 2軸の構成）
  秘書からの追加指示・カテゴリ・期間指定はありません。
```

これにより、Claude の都度判断で部署規約を上書きする事故（2026-04-28 ブリーフィング誤生成事案）を防ぐ。
