...
| 調査部 | departments/investigation/ | システムの意図しない挙動・障害・設計上の不整合について、根本原因を特定し構造的な再発防止策を策定する。 |
...
| リファクタリング部署 | departments/refactoring/ | **コードが増えても保守コストが線形に増えないアーキテクチャを維持する。** |
