...
情報収集部の存在意義は「最新情報を届けること」。以下を守る:

- **論文検索は直近3ヶ月を優先**: `site:arxiv.org {keyword} YYYY` で年指定、かつ arXiv 新着 (`/list/cs.CL/new`) を巡回
- **ブログ・ニュースは直近2週間を優先**: 対象期間の終端を今日、始端を2週間前として WebSearch
- **古い情報を混ぜる場合は明示**: 過去の重要論文を参照する場合は「**背景**」セクションに分離し、`## 📄 注目論文` には入れない
- **前回レポートとの差分チェック**: `reports/` の直近3ファイルを読み、重複 URL は除外
