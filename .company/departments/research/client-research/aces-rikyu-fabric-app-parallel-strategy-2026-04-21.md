...
**構成**: 本文（§1-9）で論点整理と提案、付録（§10-17）で詳細資料。

**スタンス**: 並行開発が常に正解とは限らない。規律（後述5条件）を守れる場合に有効な選択肢として提示する。基盤先行が合理的なケース（規制業界でデータ品質が最優先、並行開発の組織経験が無い、等）も存在する。

---

## 判断軸 — 「並行開発の可否」は技術論ではなく5条件で決まる

この資料の結論を最初に置く。**並行開発が有効かどうかは、以下5条件を組織として実行できるかに依存する**。技術（Fabric 機能の有無）は前提を整えるだけで、成否を決めない。

1. **データアクセス層の抽象化**（Repository パターン — 付録 §17）
2. **Cosmos DB 文書スキーマ／命名規則／ID 採番の統一**
3. **個人情報の削除フローと権限マッピングの事前設計**
4. **バイナリ・ベクトルの分離**（Blob Storage / Azure AI Search）
5. **Data Contract の NTTデータとの書面合意**

→ この5条件が組織として実行できる見通しが立つなら、並行開発は有効な選択肢。
→ 実行の見通しが立たないなら、基盤先行が合理的な判断となる。

詳細は付録 §16。本文はこの判断軸を前提に論点整理と実装指針を示す。
...
→ **「アプリ先に作って、後から Fabric を被せる」運用を Microsoft が公式にサポートするパターンは揃っている**。ただし技術的な土台があることと、実案件での手戻りが小さく済むことは別の話。**実務上の手戻りの大小は、判断軸の5条件（§16）で示した設計規律に強く依存する**。つまり「技術的には再利用しやすい土台がある」と「実際に手戻りが小さくなる」の間にはギャップがあり、それを埋めるのが規律。
...
本 PJ の前提（**Azure 上のアプリ、Cosmos DB、Azure AI Search、Azure OpenAI**）で「先にアプリを作って、後から Fabric を導入する」とき、何が残り何が捨てられるかを層別に整理する。

**数値の取り扱い注意**: 以下に出てくる割合（例：大半、限定的）は、同種案件の経験則レンジであり、プロジェクト固有条件（既存コード量、規律の実行度合い）で大きく変わる。規律（§16）が守られている前提での定性的な傾向として読む。
...
**→ 本 PJ のようなアプリ構成では、アプリ構築工数の大半がここに属する（同種案件の経験則）。** これらは基盤の有無に関係なく作れる。
...
問題点:
- **現場が触れる業務改善価値の立ち上がりが遅い**（基盤整備中にも品質改善や可視化準備は進むが、業務アプリとしての利用者価値は出にくい）
- データモデルが "机上設計" のまま確定してしまう可能性
- アプリ要件とのズレが発覚した時点で Gold 層の作り直し
...
利点:
- **現場が触れる業務改善価値の立ち上がりが早い**（3 ヶ月前後で MVP が現場で使われる状態）
- データモデルが "使われる順" で磨かれる = 机上設計の無駄が出にくい
...
**従来の「基盤先行」は、ETL 中心時代の合理的な選択だった**。それが絶対解でなくなったのは、2020年代後半の技術変化による。以下は "基盤先行を否定する" のではなく "並行という選択肢の技術的前提が整った" ことの背景整理。
...
→ **「基盤を完成させてからアプリ」は従来は合理的だったが、Lakehouse / Mirroring アーキテクチャの普及で再評価の余地が生まれた**。並行開発も選択肢として技術的に成立する。どちらを選ぶかは組織条件（§16 の5条件）で決める。
...
- データソース: **既存業務 DB（本 PJ では Cosmos DB。Azure SQL DB 併用の場合も同じ構造）** + Azure Blob Storage + Azure AI Search
- **必須規律**: データアクセス層を Repository パターンで抽象化、Entra ID で認証、Event Grid で主要イベント発行
- 並行して Fabric: Capacity 確保、ワークスペース設計のみ
- **ゴール**: 業務現場でアプリを触ってもらい、UX とデータ要件を明確化

### Phase 1（3〜6ヶ月）: Fabric Bronze 層で既存 DB を取り込む
- **Cosmos DB（および併用する Azure SQL DB）を Fabric Mirroring で Bronze 層にミラー**
...
### 想定工数と "捨てる部分"（同種案件の経験則レンジ）
- 全体工数: 約 12 ヶ月規模
- Fabric 導入で "捨てる" コード: 自前集計スクリプト、自前データマートなど、**集計の副産物が中心**。本質的な業務ロジックは残る
- "作り直す" 工数: **限定的**（主にデータアクセス層の実装差し替えと分析画面の接続先変更）

**注意**: 上記は同種案件の経験則であり、既存コードの状態・規律の実行度で大きく変わる。PJ 固有の見積もりは Phase 0 の設計レビューで精緻化する。

→ 「完成を待つ」ルートと比較した場合、**現場が触れる業務改善価値の立ち上がりが早い** のが並行開発の最大のメリット。捨てる部分が「少ない」というより「本質業務ロジック以外に限定される」と捉える方が正確。
...
- 技術部門 → 切り口 A（事実で論理的に）
- 予算・経営管理 → 切り口 B（コスト面で）
- 事業部門・現場 → 切り口 C（スピード・Time-to-Value で）
...
本文（§1-9）では論点整理と提案を示した。以降は実装指針・市場動向・技術詳細を付録として収録する。経営・事業寄りの相手には本文まで、技術寄りの相手には付録まで共有する想定。

| 付録 | 内容 | 対象読者 |
|---|---|---|
| §10 | 用語集 | 全員 |
| §11 | LLM/エージェント特有の要素 | 技術寄り |
| §12 | 権限管理の具体 | 技術・セキュリティ |
| §13 | その他見落としやすい項目 | 実装担当 |
| §14 | 2026 年データ基盤最新動向 | 技術・経営両方 |
| §15 | 本資料のハイライト | 全員（要約） |
| §16 | **地雷リスト & Phase 0 チェックリスト（判断軸の詳細）** | 実装担当・意思決定者 |
| §17 | Repository パターン解説 | 技術寄り・実装担当 |
...
→ Data Contract を実行できる組織では、並行開発が有力な選択肢となる。一方 Data Contract の運用が難しい組織では、従来の基盤先行が安全な選択となる。つまり **「基盤ができるまでアプリを止める」かどうかは、Data Contract の実行能力に依存する意思決定**。
...
### 14.6 このセクションのまとめ — 議論の論点を固定する3つの事実

1. **Fabric 自身が「既存業務 DB を後から取り込む」ことを公式機能として提供し、SAP/Oracle までGAしている**（2026年3月）
2. **Iceberg の台頭で、プラットフォーム選択は将来変更可能な柔軟性が高まっている**（Fabric = 不可逆なロックイン、という評価は現在の技術状況では当てはまりにくい）
3. **Data Contract の書籍化・エンタープライズ事例公開で、"並行開発が選択肢として広まりつつある" ことが公開情報で裏付けられる**

---

## 15. 本資料のハイライト — 説明時に強調する7点
...
### 16.6 説明時に使うロジック — 慎重派の懸念を取り込んだうえで前提条件を明示する
