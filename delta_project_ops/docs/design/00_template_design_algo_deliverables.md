...
- **相互参照**: 3つのテンプレート間を ID（`DEC-XXX`, `H-XXX`）で連携し、情報の重複を避ける
...
| 関連 | 意思決定ID（`DEC-XXX`）や検証記事への参照 | 該当時 |
...
- **変更内容**: {変更前→変更後の概要}
- **関連**: [DEC-{XXX}](02_decision_log.md#dec-xxx)
...
| 判断 | 理由 |
|------|------|
| 「ダッシュボード」ではなく「変更ログ」に徹する | 検証進捗は全体設計で管理。推進履歴は「なぜ計画が変わったか」のトレーサビリティを残す役割 |
...
プロジェクトの**ゴールと計画**を言語化したドキュメント。「プロジェクトの成功とは何か」を定義し、そこに至る計画を纏める。検証の進捗管理もここで行う。

プロジェクトは正確に計画された道を着実に実行することでゴールに到達できる。このドキュメントは「将来何をする必要があるか」「現在正しい位置にいるか」を知るためにある。
...
|------|------|
| 納品物 | 何を納品するか |
| 品質 | どのレベルの品質を求めるか（PJフェーズに応じて妥当な水準） |
...
| 要素 | 説明 |
|------|------|
| 検証項目と現在のステータス | 各検証の進捗（未検証/検証中/検証済/棄却） |
| 評価結果 | 評価タイプに応じた結果の記録（§1.4 参照） |
...
- [ ] 納品物に抜け漏れがないか
- [ ] 品質は PJ フェーズと比較して妥当か（低品質/過剰品質になっていないか）
- [ ] 解きたい課題が書かれているか
- [ ] スコープは解きたい課題を包含しているか
...
フォーマットは PJ ごとに自由。上記の「必要な要素」と「セルフチェックリスト」を満たしていればよい。以下は構成例：
...
| 判断 | 理由 |
|------|------|
| 「前提→ゴール→計画」の3層構造 | 「なんのためのPJか」を見失わないために、最終ゴールからブレイクダウンする構造 |
| 「未知の領域・情報設計」を追加 | 仮説を立てるのに必要な情報がない場合や、課題自体がわかっていない場合に対応。PJ開始時に全てが明確とは限らない |
| 検証進捗を全体設計に配置 | 推進履歴は変更ログに徹し、「今どこにいるか」は全体設計（ゴールと計画のドキュメント）で管理 |
| セルフチェックリストを設置 | 必要な要素の抜け漏れを防ぐ。フォーマットは自由にしつつ品質を担保する仕組み |
...
| ID | 日付 | テーマ | 決定内容 | 関連検証 |
|----|------|--------|---------|---------|
| DEC-001 | {date} | {theme} | {summary} | H-{XXX} |
...
3. **全体設計**の関連セクションを更新（検証ステータス、スコープ、リスク等）
