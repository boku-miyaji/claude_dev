    ...
    <>
      ...
      <Section title="データ鮮度マップ — いつ・何が・なぜ更新されるか">
        ...
        <Tbl headers={['作業', '画面', '頻度', '効く分析']} rows={[
          ...
          ['Roots 自然文入力', 'Roots タイムライン各カードの「自然文で書く」', '気が乗った時', 'ステージ指定＋長文で一括入力。AIが軸別（価値観/家庭/嬉しかった/苦しかった/転機/仕事/人間関係）に分解して複数エントリを自動保存。Q&A形式より思い出しやすい人向け'],
          ['Roots ステージ分割（動的カテゴリ）', 'Rootsタイムライン「細かく分ける」（5件以上で表示）', '情報が溜まった時', 'life_story_user_stages でユーザーごとにステージ管理。社会人期など情報が多いステージをAIが「1社目（会社名）」「○年目」等に分割提案。承認で life_story_entries.stage を新キーに移行。preset (幼少期〜最近8個) + custom のハイブリッド構造'],
        ]} />
    ...
    <>
      ...
      <Section title="部署サイクル設計 — いきなり実行しない">
        ...
        <Tbl headers={['部署', 'サイクル構造', '特徴的なチェックポイント', '仕様レベル']} rows={[
          ...
          ['情報収集', '収集→スコアリング→示唆抽出→intelligence_suggestions INSERT→次回収集', 'いいね/クリックの暗黙的FB + スコア減衰で過学習防止 + focus-youへの示唆は /insights Suggestions タブで「Requestsに追加」/「実装する」/却下/実装済 管理（tasks連携）', 'VERY HIGH'],
    ...
    → POST /functions/v1/ai-agent  mode=completion
      ...
      <Section title="AI Chat ツール一覧（Edge Function: ai-agent）">
        ...
        <Tbl headers={['ツール名', '機能', '検索方式', '用途']} rows={[
          ...
          ['suggestions_manage', 'focus-you示唆管理', 'Supabase filter (status)', 'intelligence_suggestions テーブル。new→checked(Requestsに追加・tasks連携)→adopted(実装確定)/rejected→implemented、またはdismissed。ボタン: 未対応→「Requestsに追加」→Request追加済→「実装する」/「却下」'],
