...
export function Dreams() {
  ...
  const [detections, setDetections] = useState<Array<{ description: string; metadata: { dream_id?: number; dream_title?: string | null; confidence?: string; reason?: string } | null; created_at: string }>>([])
  ...
  // Load recent dream detections (past 7 days, generated by narrator-update cron)
  useEffect(() => {
    const since = new Date(Date.now() - 7 * 86400000).toISOString()
    supabase
      .from('activity_log')
      .select('description, metadata, created_at')
      .eq('action', 'dream_detected')
      .gte('created_at', since)
      .order('created_at', { ascending: false })
      .limit(5)
      .then(({ data }) => { if (data) setDetections(data as typeof detections) })
  ...
  return (
    <div className="page">
      ...
      {/* 最近の気づき — weekly dream detection batch が activity_log に書き込む */}
      {detections.length > 0 && (
        <div className="section" style={{ marginBottom: 16 }}>
          <div className="section-title">最近の気づき（過去7日 / 自動検出）</div>
          <Card>
      ...
      {/* Wishlist グリッド (spec準拠: 金額・優先度バッジ・カテゴリ) */}
      {wishlist.length > 0 && (
        <div className="section" style={{ marginBottom: 20 }}>
          <div className="section-title" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span>ほしい物 (Wishlist)</span>
