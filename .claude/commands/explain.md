...
対象ディレクトリの構造を `tree` で取得し、配下の **全ファイル** を Read で読み込んで分析する。
まず、ディレクトリの内容がドキュメント中心かコード中心かを判別し、適切なフローに分岐する。

**判別基準:**
- `.md`, `.txt`, `.rst`, `.adoc` 等のドキュメントファイルが過半数 → **ドキュメントディレクトリ** (2B-docs)
- `.ts`, `.js`, `.py`, `.go` 等のコードファイルが過半数 → **コードディレクトリ** (2B-code)
- 混在の場合は両方の観点で分析する

---

#### 2B-docs. ドキュメントディレクトリの場合

配下の **全ドキュメントファイルを読み込み**、それぞれの内容を詳細に分析する。

1. **全体概要（エグゼクティブサマリー）**:
   - このドキュメント群は全体として何を説明しているか（プロジェクト全体のスコープ）
   - 対象読者は誰か（開発者 / PM / デザイナー / 運用チーム / ステークホルダー等）
   - ドキュメントの充実度（十分 / 概要レベル / 未整備）

2. **ディレクトリツリー**: tree コマンドの出力で全体構造を表示

3. **各ドキュメントの詳細解説**:
   配下の全ドキュメントファイルについて、**それぞれ** 以下を分析する。
   `<details>` で折りたたみにし、ファイルごとに展開できるようにする。

   | 項目 | 内容 |
   |------|------|
   | **ファイル名** | パスを表示 |
   | **目的・役割** | このドキュメントは何を説明しているか（3-5文で詳細に） |
   | **対象読者** | 誰が読むべきか |
   | **セクション構成** | 主要な見出し・セクションのアウトライン |
   | **要点まとめ** | ドキュメント内の重要ポイントを箇条書き（5-10項目） |
   | **定義されている用語・概念** | ドキュメント内で定義・説明されているキーワード |
   | **他ドキュメントへの参照** | 相互リンクや言及している他のファイル |

4. **ドキュメント構成マップ（テーブル）**:

   | ファイル | 説明内容 | 対象読者 | 重要度 |
   |---------|---------|---------|--------|
   | `1-README.md` | プロジェクト概要と技術スタック | 全員 | 高 |
   | `3-ARCHITECTURE.md` | システムアーキテクチャの設計 | 開発者 | 高 |
   | ... | ... | ... | ... |

5. **トピックカバレッジ（何が書かれているか）**:
   ドキュメント群全体でカバーしているトピックを洗い出し、どのドキュメントで説明されているかマトリクスで表示。

   | トピック | カバーしているドキュメント | 詳しさ |
   |---------|------------------------|--------|
   | 技術スタック | `README.md`, `ARCHITECTURE.md` | 詳細 |
   | デプロイ手順 | `DEPLOYMENT.md` | 概要のみ |
   | セキュリティ | （なし） | 未記載 |

6. **ドキュメント間の関係マップ（Mermaid）**:
   相互参照・読み順を Mermaid で可視化する。

   ```
   graph TD
     A[README] -->|前提知識| B[ARCHITECTURE]
     B -->|詳細| C[API Reference]
     A -->|セットアップ| D[GETTING STARTED]
   ```

7. **推奨読み順（リーディングガイド）**:
   初めて読む人にとって最適な読み順を提案する。
   - Step 1: まず `README.md` でプロジェクト概要を把握
   - Step 2: 次に `ARCHITECTURE.md` で設計思想を理解
   - Step 3: ...

8. **ギャップ分析（カバーされていないトピック）**:
   一般的なソフトウェアドキュメントとして期待される項目のうち、
   まだ文書化されていない、または記述が薄いトピックを指摘する。
   - 不足している可能性があるドキュメント
   - 既存ドキュメントで深堀りが必要な箇所

9. **用語集（横断）**:
   ドキュメント群全体で使われている重要な専門用語を横断的に収集し、
   定義と初出ドキュメントを一覧にまとめる。

---

#### 2B-code. コードディレクトリの場合
...
8. **各ファイルの詳細解説**: 全ファイルについてステップ 2A と同等の分析を行い、`<details>` で折りたたみ表示する
...
<body>
  ...
  <!-- ▼ ディレクトリの種別（ドキュメント/コード/単一ファイル）に応じて適切なセクションを選択する ▼ -->

  <h2>概要</h2>
  <div class="card">
    <p>{対象の役割・目的を簡潔に説明}</p>
    <!-- ドキュメントディレクトリの場合: 全体スコープ、対象読者、充実度も記載 -->
  </div>

  <h2>構造</h2>
  <pre><code>{tree 出力}</code></pre>

  <!-- === ドキュメントディレクトリの場合のセクション === -->

  <h2>ドキュメント構成マップ</h2>
  <table>
    <thead><tr><th>ファイル</th><th>説明内容</th><th>対象読者</th><th>重要度</th></tr></thead>
    <tbody>
      <tr><td><code>1-README.md</code></td><td>{何を説明しているか}</td><td>{誰向けか}</td><td><span class="badge">高</span></td></tr>
      <!-- 全ドキュメント分の行を生成 -->
    </tbody>
  </table>

  <h2>各ドキュメントの詳細解説</h2>
  <p>各ドキュメントの内容を展開して確認できます。</p>
  <!-- 全ドキュメントについて details で折りたたみ -->
  <details>
    <summary>{ファイル名} — {一行要約}</summary>
    <div>
      <h3>目的・役割</h3>
      <p>{このドキュメントは何を説明しているか、3-5文で詳細に}</p>
      <h3>セクション構成</h3>
      <ul>
        <li>{見出し1}</li>
        <li>{見出し2}</li>
  ...
  <h2>トピックカバレッジ</h2>
  <p>ドキュメント群全体でカバーしているトピックと、各トピックがどのドキュメントで説明されているかの一覧です。</p>
  <table>
    <thead><tr><th>トピック</th><th>カバーしているドキュメント</th><th>詳しさ</th></tr></thead>
    <tbody>
      <tr><td>{トピック名}</td><td><code>{ファイル名}</code></td><td><span class="badge">{詳細/概要/未記載}</span></td></tr>
    </tbody>
  </table>
  ...
  <div class="flow-step">
    ...
    <div class="flow-step-content"><strong>{ファイル名}</strong> — {なぜ最初に読むべきか}</div>
  </div>
  <div class="flow-step">
    <div class="flow-step-num">2</div>
    <div class="flow-step-content"><strong>{ファイル名}</strong> — {次に読む理由}</div>
  </div>
  <!-- 全ドキュメント分 -->

  <h2>ギャップ分析</h2>
  <p>一般的なソフトウェアドキュメントとして期待される項目のうち、まだ文書化されていない・記述が薄いトピックです。</p>
  <div class="key-point">{不足しているドキュメントや深堀りが必要な箇所}</div>

  <h2>用語集（横断）</h2>
  <table>
    <thead><tr><th>用語</th><th>定義</th><th>初出ドキュメント</th></tr></thead>
    <tbody>
      <tr><td><strong>{用語}</strong></td><td>{定義}</td><td><code>{ファイル名}</code></td></tr>
    </tbody>
  </table>

  <!-- === コードディレクトリ or 単一ファイルの場合のセクション === -->
  <!--
  <h2>詳細解説</h2>
  <details open>
