...
```bash
# バグ修正（Issue ID + 症状の説明）
/7-fix 42 ログイン画面でメールアドレスのバリデーションが効いていない

# 機能修正（Issue ID + 修正したい内容）
/7-fix 55 日記一覧のソート順が作成日時ではなく更新日時になっている

# 詳細な指示付き
/7-fix 68 APIレスポンスのページネーションで最終ページの件数が正しくない。totalCountは正しいがitemsが重複している
```
