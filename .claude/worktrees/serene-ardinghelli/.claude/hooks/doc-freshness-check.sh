...
# blueprint.html / docs/ai-features/ が実装と乖離していないか検証
...
# --- ドキュメント存在チェック ---
check "blueprint.html" "ai-features" "blueprint AI機能セクション"
check "blueprint.html" "ai-emotion" "blueprint 感情分析"
check "blueprint.html" "ai-self" "blueprint 自己分析"
check "blueprint.html" "ai-briefing" "blueprint 朝の一言"
...
# --- blueprint.html の最終更新日チェック ---
if [ -f "blueprint.html" ]; then
  # Check if any AI hook is newer than blueprint.html
  HOW_MTIME=$(stat -c %Y blueprint.html 2>/dev/null || echo 0)
  for hook_file in src/hooks/useEmotionAnalysis.ts src/hooks/useSelfAnalysis.ts src/hooks/useMorningBriefing.ts src/hooks/useDreamDetection.ts src/hooks/useWeeklyNarrative.ts; do
    if [ -f "$hook_file" ]; then
      ...
      if [ "$HOOK_MTIME" -gt "$HOW_MTIME" ]; then
        ...
        WARNINGS="${WARNINGS}  ⚠ $HOOK_NAME が blueprint.html より新しい → ドキュメント更新が必要かもしれません\n"
        ERRORS=$((ERRORS + 1))
        break
      fi
    fi
