...
| 時間トリガー集計 | バッチ (GitHub Actions) | 定刻実行・大量計算 |
| morning digest / 週次レポート | バッチ (GitHub Actions) | Hook では不可（重い・時刻依存） |

**原則**: Hook は「記録（軽量・即時）」、/company は「判断と行動」、バッチは「時刻トリガー + 大量計算」

### Batch を使うべき判断基準

Hook ではなく Batch に分類するのは以下の条件を満たす場合:

- **時刻トリガー**: 「毎朝 8 時」「毎週月曜」など、ユーザー操作と無関係に起動する
- **重い計算**: LLM API を複数回呼ぶ、大量レコードを集計するなど、数秒〜数分かかる処理
- **非インタラクティブ**: ユーザーが待っていない（応答に影響しない）

Hook に入れると会話をブロックする可能性があるため、上記はすべて GitHub Actions の cron スケジュールで実行する。
