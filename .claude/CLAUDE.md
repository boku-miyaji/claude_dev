...
- 実装後は必ず docs を最新化する。
- 実装したら必ず commit する。
- **IMPORTANT (focus-you / company-dashboard PJ 限定): TodoWriteでタスク作成時、最後のステップに「Blueprint更新確認」を必ず含める。** `company-dashboard/` 配下のコード変更に限り、`company-dashboard/src/pages/Blueprint.tsx` の該当セクションを確認・更新してからタスクをdoneにする。**他PJ（aces-rikyu-sales-proposals-poc / polaris-circuit / その他）には Blueprint.tsx を適用しない**（focus-you 固有のダッシュボード）。
