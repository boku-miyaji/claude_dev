...
| エージェント構築 | **MS Agent Framework（Azure AI Agent Service）** | Microsoft公式フレームワーク、Azure統合 |
| 自動評価 | **LLM-as-Judge (GPT-5.2) + カスタムルーブリック** | 出力品質の自動評価、ドメイン特化のルーブリック定義 |

### 7.2.1 エージェント構築方式の選定

本PoCでは、**MS Agent Framework（Azure AI Agent Service）** を採用する。

#### MS Agent Frameworkを選定した理由

1. **Azure統合**: Azure OpenAIとのシームレスな統合、認証・管理が容易
2. **Microsoft公式**: エンタープライズ向け、サポート・ドキュメントが充実
3. **エージェント機能**: Tool calling、Function calling、Agentパターンをネイティブサポート
4. **将来の拡張性**: 本番化時にそのまま移行可能、スケーラビリティも担保

#### MS Agent Framework アーキテクチャ
...
│  │  ┌─────────────────────────────────────────────────────────┐  │  │
│  │  │ Agent Definition                                         │  │  │
│  │  │ ├─ ニーズ推定Agent: 顧客情報→ニーズリスト抽出          │  │  │
│  │  │ ├─ ソリューション推薦Agent: ニーズ→商材提案            │  │  │
│  │  │ └─ 自動評価Agent: 出力品質の評価                        │  │  │
│  │  └─────────────────────────────────────────────────────────┘  │  │
│  │         ▼                                                      │  │
│  │  ┌─────────────────────────────────────────────────────────┐  │  │
│  │  │ Tools / Functions                                        │  │  │
│  │  │ ├─ 商材検索Tool: 商材カタログからの検索                 │  │  │
│  │  │ ├─ 類似事例Tool: 過去提案事例の参照                     │  │  │
│  │  │ └─ 評価Tool: ルーブリックに基づく評価実行               │  │  │
│  │  └─────────────────────────────────────────────────────────┘  │  │
│  │         ▼                                                      │  │
│  │  ┌─────────────────────────────────────────────────────────┐  │  │
│  │  │ Tracer Layer（抽象化）                                   │  │  │
...
class NeedsEstimatorService:
    ...
    async def estimate(
        ...
        company_info: str,
        hearing_info: str,
        management_agenda: dict = None,   # ★経営アジェンダシート
        key_person_map: dict = None,       # ★キーパーソンマップ
        knowledge: dict = None
    ) -> NeedsEstimationResult:
        """ニーズを推定（経営アジェンダ・キーパーソンマップを考慮）"""
        with self.tracer.span("needs_estimation") as span:
            # ユーザープロンプト構築
            user_prompt = self._build_user_prompt(
                company_info, hearing_info, management_agenda, key_person_map, knowledge
            )
            span.set_input({
                "company_info": company_info,
                "hearing_info": hearing_info,
                "management_agenda": management_agenda,
                "key_person_map": key_person_map
            })
    ...
    def _build_user_prompt(
        ...
        return template.format(
            company_info=company_info,
            hearing_info=hearing_info,
            management_agenda=management_agenda or "経営アジェンダなし",
            key_person_map=key_person_map or "キーパーソンマップなし",
...
| v1.1 | 2026-01-25 | 伴奏AI対応: NeedsEstimatorServiceに経営アジェンダシート・キーパーソンマップパラメータ追加 |
